...
/*find all elements with classes who start with beach_class_ and save them in array. These are the buttons for informations. Then
a modal window for each is goint to open*/
/**/
var found = [];

$('div[class*="beach_class_"]').each(function() {
  var classes = this.className.split(/\s+/),
  $this = $(this);

  $.each(classes, function(i, name) {
      if (name.indexOf('beach_class_') === 0) {
...
//alert( index + ": " + value +" length: "+value.length+"substr:"+value.substring(value.length-2,value.length)+" "+"\"."+value+"\"");
//var classitem="\"."+value+"\"";
var classitem="."+value.substring(value.length-2,value.length);
var iditem="#beach_button_id_"+value.substring(value.length-2,value.length);
var idmodalitem="#beach_details_modal_"+value.substring(value.length-2,value.length);
//alert("iditem:"+iditem);
//alert("classitem:"+classitem);
//alert("iditem:"+iditem);
//alert("idmodalitem:"+idmodalitem);
$(iditem).click(function () {
//alert("Hello from button:"+iditem);
$(idmodalitem).dialog({
                    width: 800,
                    height: 600,
                    modal: true,
...
var foundbtnmaps = [];
$('a[class*="btnShow_"]').each(function() {
  var classes = this.className.split(/\s+/),
  $this = $(this);

  $.each(classes, function(i, name) {
      if (name.indexOf('btnShow_') === 0) {
          //$this.removeClass(name);
          foundbtnmaps.push(name);
      }
  });
});

console.log(foundbtnmaps);

/**/
$.each(foundbtnmaps, function( index, value ) {
var classitem=".btnShow_"+value.substring(value.length-2,value.length);

var dialogiditem="#dialog_"+value.substring(value.length-2,value.length);
var dvmapiditem="#dvMap_"+value.substring(value.length-2,value.length);
var locinfoid="#loc_info_"+value.substring(value.length-2,value.length);
console.log(classitem+" "+dialogiditem+" "+dvmapiditem+" "+locinfoid);

//start of button click handler
$(classitem).click(function () {
//alert("Hey from map button"+classitem);

var locinfo=$(locinfoid).val();
values=locinfo.split(';');
var latit=values[0];
var longit=values[1];
...
$(dialogiditem).dialog({
            ...
            open: function () {
                ...
                var map = new google.maps.Map($(dvmapiditem)[0], mapOptions);
            }
...
/*
$.each(foundmaps, function( index, value ) {
//alert( index + ": " + value +" length: "+value.length+"substr:"+value.substring(value.length-2,value.length)+" "+"\"."+value+"\"");
//var classitem="\"."+value+"\"";


//var classitem="."+value.substring(value.length-2,value.length);
var classitem=".map_canvas_"+value.substring(value.length-2,value.length);
//var mapmodalitem="#beach_details_modal_"+value.substring(value.length-2,value.length);
...
<body>
<?php include 'show_images.php';?> <!--include external php script for image retrieving from database-->
<?php include 'hidden_modal.php';?> <!--include external php script for setting modal windows-->
...
<?php print_modal_windows()?>        
<!-- Hidden modal forms Agioi_Apostoloi  static
    
     <div id="beach_details_modal_Agioi_Apostoloi" title="Beach Details Agioi Apostoloi" style="display:none;">
     ...
     </div>--><!--end of hidden modal window agioi apostoloi static-->
